...
{
    // Ensure camera does not exceed the right boundary
    ...
    // Ensure camera does not exceed the bottom boundary
    ...
    // Ensure camera does not exceed the left boundary
    if (_camera.position.x < _camera.bounds.x)
    {
        _camera.position.x = _camera.bounds.x;
    }
    // Ensure camera does not exceed the top boundary
